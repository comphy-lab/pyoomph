...
| [subexpression_unit_analysis_stall.md](subexpression_unit_analysis_stall.md) | Code generation never finishing on a UNIFAC mass-transfer residual under `azimuthal_stability=True`. The DAG-as-tree mapper family behind it, the placeholder masking that made the unit analysis flat in the nesting depth, the one node type that must not be masked — and the two walls underneath it that are still open: GiNaC's `collect_common_factors` on a 14806-term sum, and the code writer. |
| [droplet_azimuthal_experiment_series.md](droplet_azimuthal_experiment_series.md) | Handover: the azimuthal m-scan over the measured droplet-instability data set, blocked with 0 of 14 rows. Wall one is **fixed**: the azimuthal split's memo shared one key between an exact `-2` and an inexact `-2.0`, and GiNaC's `ex::compare` unified the two in place, sending `power::real_part` into polar form and a dimensional basis into an `atan2`. Wall two is open: a transient that never reaches the target time at the eigen script's thousand-times-finer contact line. |
